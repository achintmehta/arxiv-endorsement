...
Use this exact four-line format, with one field per line:
...
1. The file must contain exactly these four fields, in this order: `LinkedIn`, `Paper`, `Subject`, `EndorsementCode`.
...
6. `EndorsementCode` must be the six-character alphanumeric code generated by arXiv for the endorsement request.
7. LinkedIn verification is still checked manually. The automated check only validates that the URL has the right shape.
